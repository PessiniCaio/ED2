...
int main() {
    ...
    FILE *arquivo = fopen("Entrada_busca_binaria.csv", "r");
    if (arquivo == NULL) {
    ...
    if (resultado_tradicional != -1) {
        printf("Elemento encontrado na posição %d (iterativa).\n", resultado_tradicional);
    } else {
        printf("Elemento não encontrado (iterativa).\n");
    }

    // Busca recursiva
    int resultado_recursivo = busca_binaria_recursiva(vetor, 0, tamanho - 1, chave);
    if (resultado_recursivo != -1) {
        printf("Elemento encontrado na posição %d (recursiva).\n", resultado_recursivo);
    } else {
        printf("Elemento não encontrado (recursiva).\n");
    }
